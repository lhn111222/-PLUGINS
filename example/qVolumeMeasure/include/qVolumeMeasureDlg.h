...
private slots:
    ...
    // 新声明（匹配信号参数）：
    //void handlePickedItem(ccHObject* entity, int itemIdx, int x, int y);
     //void handlePickedItem(ccHObject* entity, unsigned itemIdx, const CCVector3& P);
    void handlePickedItem(ccHObject* entity, unsigned itemIdx, int x, int y, const CCVector3& P, const CCVector3d& uwv);
   // void onGLWindowPickingEvent(ccHObject* entity, unsigned itemIdx, const CCVector3& P);
...
// 在 qVolumeMeasureDlg.h 中修改成员变量声明：
